module Oz
  class RouterStore
    ...
    def self.store_page_blueprint(page_name, parents)
      @stored_blueprints[page_name] = PageBlueprint.new(page_name, parents)
    end
    ...
    def self.store_id_element(page, element_type, id_hash, action)
      ...
      store_element(page, options_hash, action, :id)
    end

    def self.store_wait_element(page, element_type, id_hash, action)
      options_hash = { element_type: element_type }.merge(id_hash)
      store_element(page, options_hash, action, :wait)
    end
